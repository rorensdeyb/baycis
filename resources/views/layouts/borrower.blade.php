    ...
    @php
        ...
        try {
            if (class_exists(\App\Models\Notification::class)) {
                ...
                $borrowerNotifs = \App\Models\Notification::where('user_id', auth()->id())->latest()->take(5)->get();
            }
        } catch (\Throwable $e) {}
    @endphp
...
<div class="admin-layout d-flex">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('borrower.returns') }}" class="nav-item {{ request()->routeIs('borrower.returns') ? 'active' : '' }}">
                ...
                @if($activeBorrows > 0)
                    <span class="badge bg-danger rounded-pill ms-auto">{{ $activeBorrows }}</span>
                @endif
            </a>
        ...
        <div class="sidebar-footer">
            <button class="nav-item text-danger w-100 justify-content-center" data-bs-toggle="modal" data-bs-target="#logoutModal" style="border: none; background: transparent; padding: 12px; gap: 8px;">
                <i class="bi bi-box-arrow-right"></i> <span class="fw-bold">Sign out</span>
            </button>
        </div>
    </aside>
        ...
        <header class="top-navbar d-flex justify-content-between align-items-center">
            ...
            <div class="top-actions ms-auto d-flex align-items-center gap-2">
                ...
                <div class="dropdown d-inline-block">
                    ...
                    <div class="dropdown-menu dropdown-menu-end shadow-lg" style="width: 340px; border-radius: 16px; border: 1px solid var(--border-color); background: var(--bg-surface); padding: 0; overflow: hidden; margin-top: 10px;">
                        ...
                        <div style="max-height: 300px; overflow-y: auto;">
                            @forelse($borrowerNotifs as $notif)
                                <form action="{{ route('notifications.read', $notif->id) }}" method="POST" class="m-0">
                                    @csrf
                                    <button type="submit" class="dropdown-item p-3 border-bottom text-wrap w-100 text-start" style="white-space: normal; background: {{ $notif->is_read ? 'transparent' : 'var(--accent-green-bg)' }}; border: none; cursor: pointer;">
                ...
                <!-- THEME TOGGLE -->
                <button class="icon-btn" onclick="toggleTheme()"><i id="theme-icon" class="bi bi-moon-stars"></i></button>

                <div class="d-none d-md-flex align-items-center justify-content-center user-avatar ms-2">
    ...
    <nav class="bottom-nav">
        ...
        <a href="{{ route('borrower.returns') }}" class="bottom-nav-item {{ request()->routeIs('borrower.returns') ? 'active' : '' }} position-relative">
            <i class="bi bi-arrow-return-left"></i>
            <span>Returns</span>
            @if($activeBorrows > 0)
                <span class="position-absolute top-0 start-50 translate-middle p-1 bg-danger border border-light rounded-circle" style="margin-top: 10px; margin-left: 8px;"></span>
            @endif
        </a>

        <a href="{{ route('borrower.account') }}" class="bottom-nav-item {{ request()->routeIs('borrower.account') ? 'active' : '' }}">
        ...
        <div class="modal-content" style="background-color: var(--bg-surface); color: var(--text-primary); border-radius: 16px; border: 1px solid var(--border-color);">
            <div class="modal-header border-0 pt-4 px-4 pb-0">
                <h5 class="modal-title fw-bold text-danger d-flex align-items-center gap-2">
                    <div style="width: 32px; height: 32px; background: rgba(220, 53, 69, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center;"><i class="bi bi-box-arrow-right"></i></div>
                    Sign Out
                </h5>
...
<script>
    ...
    function toggleTheme() {
        ...
        if (themeIcon) {
            newTheme === 'dark' ? themeIcon.classList.replace('bi-moon-stars', 'bi-sun') : themeIcon.classList.replace('bi-sun', 'bi-moon-stars');
        }
    }
...
(function() {
    ...
    async function submitPin() {
        ...
        try {
            ...
            if (res.ok) {
                // Success — remove overlay with animation
                const card = document.getElementById('pinSetupCard');
                card.style.transform = 'scale(1.05)';
                card.style.opacity = '0';
                card.style.transition = 'all 0.3s ease';
                setTimeout(() => {
                    document.getElementById('pinSetupOverlay').remove();
                }, 350);
            } else {
                flashError('p2');
